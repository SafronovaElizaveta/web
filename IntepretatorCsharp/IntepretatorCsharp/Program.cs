...
namespace IntepretatorCsharp
{
    public class Intepreter
    {
        ...
        public void Emit(int t, int tval)
        {
            ...
            switch (t)
            {
                ...
                case INDEX:
                    ...
                    break;
                case METKAJF:
                    OPSMass[OPSCounter].Element = "<metka jf>".ToCharArray();
                    OPSMass[OPSCounter].Type = OPSType.METJF;
                    OPSCounter++;
                    //Console.WriteLine("<jf>");
                    break;
                case METKAJ:
                    OPSMass[OPSCounter].Element = "<metka j>".ToCharArray();
                    OPSMass[OPSCounter].Type = OPSType.METJ;
                    OPSCounter++;
                    //Console.WriteLine("<j>");
                    break;
                case READ:
                    OPSMass[OPSCounter].Element = "<read>".ToCharArray();
                    OPSMass[OPSCounter].Type = OPSType.RE;
                    OPSCounter++;
                    //Console.WriteLine("<r>");
                    break;
                case WRITE:
                    OPSMass[OPSCounter].Element = "<write>".ToCharArray();
        ...
        public void Match(int t)
        {
            ...
            {
                Console.WriteLine("Sintax error! " + "line: " + LineNumber + "  symbol: " + ss/2+"  Expected: "+LookAhead+"  Now: "+t);
                Environment.Exit(0);
            }
        }
                ...
                switch (OPSMass[z].Type)
                {
                    ...
                    case OPSType.SIGN:
                        ...
                        switch (t)
                        {
                            ...
                            case '/':
                                firstElement = stack.Pop();     // Правое значение
                                secondElement = stack.Pop();    // Левое значениt
                                switch (secondElement.Type)
                                {
                                    case OPSType.IDE:
                                        switch (firstElement.Type)
                                        {
                                            case OPSType.IDE:
                                                if (Variables[firstElement.Value].Token == 0)
                                                {
                                                    Console.WriteLine("Attempt to divide by 0");
                                                    Environment.Exit(0);
                                                }
                                                res = Variables[secondElement.Value].Token /
                                                      Variables[firstElement.Value].Token;
                                                break;
                                                    
                                                
                                            case OPSType.MAS:
                                                if (MassesTable[Passport[firstElement.Value].Mass].Element[Passport[firstElement.Value].Element] == 0)
                                                {
                                                    Console.WriteLine("Attempt to divide by 0");
                                                    Environment.Exit(0);
                                                }
    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            string fileName = "test4.txt"; // имя исходного файла
